...
The image in [Dockerfile](Dockerfile) runs anywhere that runs a container. It
serves the API and the built browser app together on one origin, which is what
keeps the session cookie a plain `SameSite=Lax` one — a split deployment
would need `SameSite=None`, which Safari and Brave restrict, producing
sign-outs that reproduce only on someone else’s machine.

**The contract, whatever the host:**

| | |
| --- | --- |
| Build | `./Dockerfile`, no build args |
| Listens on | `$PORT` (defaults to 8080) |
| Health check | `GET /health` |
| Needs | `DATABASE_URL`, `SESSION_SECRET`, `WEB_ORIGIN`, `NODE_ENV=production` |

**Put the service in the same region as the database.** It is the app-to-database
hop that matters, not the user-to-app one: a page load is one crossing from the
browser, while the endpoints behind it make dozens against Postgres and ingesting
a manuscript makes hundreds.

### 1. Database

Postgres on [Neon](https://neon.tech) — free, no card, and it survives the app
being redeployed or moved between hosts.

Hosts on free tiers rarely give you a shell, but Neon is reachable from
anywhere, so migrate and seed from your own machine:
...
never invent one or prompt. And `.env` does not override a variable already set
in the shell, so those commands genuinely reach Neon rather than the local
database.
...
`WEB_ORIGIN` is only read by the CORS allowlist, which a single-origin
